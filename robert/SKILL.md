...
בנוסף למלונות יש טאב "הוצאות" בגיליון ומסך "תקציב" באפליקציה. כל הוצאה: `{"category":"טיסה|מזומן|תחבורה|אטרקציות|ביטוח|אחר","title":"...","date":"YYYY-MM-DD","date2":"(חזור, לטיסה)","route":"TLV → BKK","price":4200,"currency":"ILS","account":"צחי|שירן","paid":true,"confirmation":"...","notes":"..."}`. `paid` = האם כבר שולם (ברירת מחדל true; מזומן תמיד true). "סמן את הטיסה כשולמה" → `updateExpense` עם `{"paid":true}`.
